...
sub checkOptions {
    my %opts;
    getopts('hz:r:o:n:', \%opts);
    my ($help, $contig, $emm_DB, $outDir, $outName);
    ...
    return ($help, $contig, $emm_DB, $outDir, $outName);
}
...
USAGE
emm_typer.pl -z <assembled contig: fasta> -r <reference databases directory: file path> -o <output directory name: string> -n <output name prefix: string>  [OPTIONS]
...
my ($help, $contig, $emm_DB, $outDir, $outName) = checkOptions( @ARGV );
...
if (glob("$emm_DB/blast_frwd_primr-nucl_DB*")) {
    system("blastn -db $emm_DB/blast_frwd_primr-nucl_DB -query $contig -outfmt 6 -word_size 4 -out contig-vs-frwd_nucl.txt");
} else {
...
if ($best_frwd_len == 19 && $best_frwd_iden >= 94.5) {
	...
	close $fh;
	#print "done\n";
	system("bedtools getfasta -fi $contig -bed emm_region_extract.bed -fo emm_region_extract.fasta");
    } else {
	...
	my $extract_emm1 = `bedtools getfasta -tab -fi $contig -bed emm_region_extract.bed -fo stdout`;
...
if ($best_emm_iden == 100 && $best_emm_len == 180) {
    ...
    my $emmType = $1;
    print $fh "$outName\t$emmType\t$best_emm_name\t$best_emm_iden\t$best_emm_len\n";
} else {
    #$best_emm_name =~ /\d+__EMM(.*)__EMM.*__\d+/;
    $best_emm_name =~ /\d+__[A-Z]+(.*)__.*__\d+/;
    my $emmType = $1;
